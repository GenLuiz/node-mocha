...
describe('user data', function(){
	it('should have +1 user count after saving', function* () {
		...
		yield data.users.save({name: 'Luiz'});
...
describe('user web', function () {
	it('should have +1 user count after saving', function* () {
		var response = yield request.get('/user').expect(200).end();

		var users = response.body;

		yield data.users.save({name: 'Luiz'});

		var newResponse = yield request.get('/user').expect(200).end();

		var newUsers = newResponse.body;
